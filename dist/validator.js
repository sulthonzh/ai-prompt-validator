...
class PromptValidator {
    ...
    validate(prompt, enabledRules) {
        const issues = [];
        const suggestions = [];
        const enabledRulesList = enabledRules || this.rules.map(rule => rule.id);
        const enabledRuleObjects = this.rules.filter(rule => enabledRulesList.includes(rule.id));
    ...
    calculateClarityScore(prompt, sentences) {
    ...
    calculateStructureScore(sentences) {
        let score = 50;
    ...
    calculateCreativityScore(prompt, words) {
    ...
    enableRule(id) {
        ...
        if (rule) {
            rule.validate = (prompt) => true;
        }
    }
    disableRule(id) {
        const rule = this.getRule(id);
        if (rule) {
            rule.validate = (prompt) => false;
        }
    }
    addCustomRule(rule) {
